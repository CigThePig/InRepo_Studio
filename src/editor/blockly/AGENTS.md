...
- **Blockly Mode state**: boolean mode flag + listeners; enter/exit orchestration wired into editor init.
- **Logic Target switching**: swap workspace file when dropdown changes, handle empty-state (no script yet).
- **Block insertion**: handle "Insert block" actions from Presets Hooks (via callback on left berry) and palette tap-to-insert.
- **Workspace serialization bridge**: save/load workspace JSON via storage API.
- **Blocks palette component**: categorized, searchable block list rendered inside the right berry in Blockly Mode.
- **Palette categories**: PALETTE_CATEGORIES definitions and Logic Target filtering helpers.
- **Right berry tab definitions for Blockly Mode**: BLOCKLY_BERRY_TABS (Blocks + Inspect).
- **Inspect/Errors panel** (`inspectPanel.ts`): script status strip, last error display with block highlight, active timer count, recent log entries. Receives updates from `blocklyCockpit.ts` via ScriptHost lifecycle events.

Does NOT own:
- Block definitions or code generators (use `/src/runtime/blockly`)
- ScriptHost or script execution (use `/src/runtime/blockly`)
- Preset UI or Presets tab content (use `/src/editor/presets`)
- Berry shell containers (use `/src/editor/panels` — rightBerry/leftBerry)
...
/src/editor/blockly/
├── AGENTS.md
├── blocklyMode.ts           ← Blockly Mode boolean state + listeners
├── blocklyWorkspace.ts      ← Workspace injection, Zelos config, change listener, lifecycle
├── workspaceManager.ts      ← Save/load orchestration, Logic Target switching, debounced auto-save (1s), empty state
├── blocklyTopBar.ts         ← Blockly Mode top bar (Back, Logic Target dropdown, Run/Stop, status indicator)
├── blocklyCockpit.ts        ← Full orchestrator: lazy-load, DOM layout, enter/exit, beforeunload save, right berry wiring
├── paletteCategories.ts     ← PALETTE_CATEGORIES (11 v1 categories), getCategoryBlocks(), isCategoryEnabled(), isCategoryVisible()
├── blocklyBerryTabs.ts      ← BLOCKLY_BERRY_TABS tab definitions (Blocks + Inspect)
├── blocksPalette.ts         ← Categorized block browser: search, tap-to-insert, advanced toggle, dependency prompts
├── blocksPaletteStyles.ts   ← Extracted CSS for blocksPalette component
├── inspectPanel.ts          ← Inspect/Errors tab content (Track 42): script status, last error, active timers, recent logs
...
- **Toolbox**: NOT used as primary palette (right berry IS the palette). Blockly's built-in toolbox is hidden/empty.
- Workspace is lazy-loaded (separate Vite chunk) — do not import Blockly at module top level outside blocklyCockpit.
...
- Blocks inserted via right berry palette: tap to insert (placed near viewport center or selection).
- Blocks inserted via Presets Hooks "Insert block" button: callback wired through left berry → blocklyCockpit → workspaceManager.
...
- Auto-save on meaningful changes (workspace change listener with 1s debounce).
...
- Run/Stop buttons must be easily reachable (top bar, thumb zone).
...
- Block insertion from Presets Hooks flows via a callback on blocklyCockpit — presets UI does not import workspace internals.
